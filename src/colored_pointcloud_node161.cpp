...
class RsCamFusion
{
  ...
  public:
    ...
    void callback(const sensor_msgs::ImageConstPtr input_image_msg,
                const sensor_msgs::PointCloud2ConstPtr input_cloud_msg)                 //输入图像 输入点云 msg格式
    {
      cv::Mat input_image;
      cv::Mat undistorted_image;
      cv_bridge::CvImagePtr cv_ptr; //通过cv::bridge转成opencv格式
...
// =================== (1) sensor_msgs to cv image   
      ...
      input_image = cv_ptr->image;          //转换之后   input_image
//==============================================      
      ...
      //pcl::fromROSMsg( const senser_msgs::PointCloud2 & , pcl::PointCloud<Point T> &) 因为下面使用的是指针,所以需要加* 来返回指针对应的值
      ...
      //reserve the points in front of the camera(z>0)  提供一个图像的深度彩色值可视化(不重要)
      ...
      {
        if(imagePoints[i].x>=0 && imagePoints[i].x<1241 && imagePoints[i].y>=0 && imagePoints[i].y<376)//图像大小
        {
          ...
          point.r = input_image.at<cv::Vec3b>(imagePoints[i].y, imagePoints[i].x)[2];//图像坐标系 先访问Y 后X 前面图像是GBR
...
//将opencv格式转为msg
    void publishImage(const ros::Publisher& image_pub, const std_msgs::Header& header, const cv::Mat image) //将cv格式的图像打包成msg
    {
      ...
      image_pub.publish(output_image); //将cv_bridge直接发布出去了
    } 
    ...
    void publishCloudtoShow(const ros::Publisher& cloudtoshow_pub, const std_msgs::Header& header,        //发布的点云格式变为PointXYZRGB格式
...
int main(int argc, char** argv)
{
  ...
  if (priv_nh.hasParam("calib_file_path") && priv_nh.hasParam("file_name"))//yaml文件path
  {
    priv_nh.getParam("camera_topic", camera_topic);                       //相机topic
    priv_nh.getParam("lidar_topic", lidar_topic);                         //lidar topic
    priv_nh.getParam("calib_file_path", config_path);                     //相机内参文件
    priv_nh.getParam("file_name", file_name);                             //yaml文件path
    priv_nh.getParam("color_distance", color_dis);                         //?
    priv_nh.getParam("show_colored_cloud", show_cloud);                     //输出点云(带RGB 信息)
    priv_nh.getParam("save_data", save_data);                               //是否保存数据
  }
  else
  {
  ...
  typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, sensor_msgs::PointCloud2>MySyncPolicy;//定义消息同步机制
  message_filters::Synchronizer<MySyncPolicy> sync(MySyncPolicy(10), camera_sub, lidar_sub);//定义类成员变量
  ...
  colored_cloud_showpub = nh.advertise<sensor_msgs::PointCloud2>("colored_cloud_toshow", 10); //创建publisher  提供topic 名称
  ros::spin();
  return 0;
